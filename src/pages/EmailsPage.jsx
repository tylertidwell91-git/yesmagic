...
function EmailsPageInner() {
  ...
  return (
    <div className="yesmagic-main admin-layout">
        ...
        {loading && <p style={{ color: 'var(--ym-muted)', marginBottom: '1rem' }}>Loading subscribers…</p>}
        {error && (
          <div className="error-message" style={{ marginBottom: '1rem' }}>
            {error}
          </div>
        )}
        {saved && <div className="success-message" style={{ marginBottom: '1rem' }}>Subscribers saved.</div>}
          ...
          <>
            <section className="admin-emails-section">
              ...
              <p style={{ color: 'var(--ym-muted)', marginBottom: '0.75rem', fontSize: '0.85rem' }}>
                This table shows everyone who has entered via the subscription form. Only one entry is allowed per
                shipping address, but you can edit names, emails, or addresses here.
              </p>
                ...
                <table className="admin-emails-table">
                  ...
                  <tbody>
                      ...
                      <tr key={s.id}>
                        <td>
                          <input
                            type="text"
                            value={s.name ?? ''}
                            onChange={(e) => updateSubscriber(s.id, 'name', e.target.value)}
                          />
                        </td>
                        <td>
                          <input
                            type="email"
                            value={s.email ?? ''}
                            onChange={(e) => updateSubscriber(s.id, 'email', e.target.value)}
                          />
                        </td>
                        <td>
                          <textarea
                            value={s.shippingAddress ?? ''}
                            onChange={(e) => updateSubscriber(s.id, 'shippingAddress', e.target.value)}
                            rows={2}
                          />
                        </td>
                      </tr>
                    ...
                    {subscribers.length === 0 && (
                      <tr>
                        <td colSpan={3} style={{ color: 'var(--ym-muted)', fontSize: '0.85rem', textAlign: 'center' }}>
                          No subscribers yet.
                        </td>
                      </tr>
                    )}
            ...
            <section className="admin-emails-section" style={{ marginTop: '2rem' }}>
              <h3>Mass email: we&apos;re live on WhatNot</h3>
              <p style={{ color: 'var(--ym-muted)', marginBottom: '0.75rem', fontSize: '0.85rem' }}>
                Sends a branded email to all subscribers letting them know that you are live streaming on WhatNot right
                now, with a button linking to your channel.
              </p>
              {WHATNOT_URL ? (
                <p style={{ color: 'var(--ym-muted)', marginBottom: '0.5rem', fontSize: '0.8rem' }}>
                  Current WhatNot URL: <code>{WHATNOT_URL}</code>
                </p>
              ) : (
                <p style={{ color: '#f97373', marginBottom: '0.5rem', fontSize: '0.8rem' }}>
                  Set <code>VITE_WHATNOT_CHANNEL_URL</code> in your Netlify env so the email can include a link.
                </p>
              )}
              <button
                type="button"
              ...
              {sendingLiveResult && (
                <p style={{ marginTop: '0.5rem', fontSize: '0.85rem', color: 'var(--ym-muted)' }}>
                  {sendingLiveResult}
                </p>
              )}
            </section>
            ...
            <section className="admin-emails-section" style={{ marginTop: '2rem' }}>
              ...
              <p style={{ color: 'var(--ym-muted)', marginBottom: '0.75rem', fontSize: '0.85rem' }}>
                Send a custom message to all subscribers. The email will use the YESMagic branding and logo, and can
